...
class Transaction(Base):
    ...
    gross_amount_pln: Mapped[float] = mapped_column(Numeric(10, 2), nullable=False)
    discount_pct_snapshot: Mapped[float] = mapped_column(Numeric(5, 2), nullable=False)
    discount_amount_pln: Mapped[float] = mapped_column(Numeric(10, 2), nullable=False)
    net_amount_pln: Mapped[float] = mapped_column(Numeric(10, 2), nullable=False)
    basket_snapshot_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("baskets.id"), nullable=False
    )
    balance_before_pln: Mapped[float] = mapped_column(Numeric(10, 2), nullable=False)
    balance_after_pln: Mapped[float] = mapped_column(Numeric(10, 2), nullable=False)
    ...
    basket_snapshot: Mapped["Basket"] = relationship("Basket", foreign_keys=[basket_snapshot_id])  # type: ignore[name-defined]
